...
pub struct PerceptionSupervisor {
    pub inner: Arc<Mutex<SupervisorInner>>,
}
...
fn sidecar_dir() -> Result<PathBuf> {
    ...
    if !dir.exists() {
        return Err(anyhow!(
            "perception sidecar dir not found at {}",
            dir.display()
        ));
    }
    Ok(dir)
...
fn build_command(dir: &PathBuf) -> Result<Command> {
    let python = which_python().ok_or_else(|| {
        anyhow!("python3 / python not found in PATH — perception sidecar unavailable.")
    })?;
    ...
    tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = reader.next_line().await {
            ...
            let frame: Value = match serde_json::from_str(trimmed) {
                Ok(v) => v,
                Err(e) => {
                    log::warn!("perception sidecar unparseable line: {} ({})", trimmed, e);
                    continue;
                }
            };
