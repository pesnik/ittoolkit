use crate::mcp_types::{
    self, CallToolParams, CallToolResult, ContentBlock, InitializeResult, ListResourcesResult,
    ListToolsResult, McpResource, McpTool, ResourcesCapability, ServerCapabilities, ServerInfo,
    ...
    loop {
        ...
        match reader.read_line(&mut buf) {
            ...
            Ok(_) => {
                ...
                let req: crate::mcp_types::JsonRpcRequest = match serde_json::from_str(line) {
                    Ok(r) => r,
                    Err(e) => {
                        write_frame(
                            &stdout,
...
fn handle(stdout: &std::io::Stdout, req: crate::mcp_types::JsonRpcRequest) {
            ...
            if let Some(id) = id {
                ...
                let result = CallToolResult {
                    content: vec![ContentBlock::Text {
                        text: format!(
                            "ittoolkit's '{}' tool is interactive: it requires user approval \
                             inside the ittoolkit app before executing. Open ittoolkit, ensure \
                             the computer-use harness is enabled, and dispatch the action from \
                             there. The MCP server exposes the catalog for planning; actual \
                             execution stays under the local user's direct supervision.",
                            params.name
                        ),
                    }],
                    is_error: Some(true),
...
fn write_frame(stdout: &std::io::Stdout, frame: &crate::mcp_types::JsonRpcResponse) {
