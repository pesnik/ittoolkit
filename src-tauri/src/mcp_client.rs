// MCP client — spawns external MCP servers and routes tool calls to them.
//
// Config file: ~/.ittoolkit/mcp-clients.json
//   {
//     "servers": {
//       "postgres-local": {
//         "command": "uvx",
//         "args": ["mcp-server-postgres", "--db-url", "postgres://…"],
//         "env": {}
//       },
//       "slack": { … }
//     }
//   }
//
// Each entry is spawned via stdio on the first call that targets it. Tool
// names are namespaced as `<server-id>__<tool>` (double underscore) when
// surfaced to the planner so the inference loop can route them back.
//
// Lifecycle: spawn lazy, idle-kill after 10 minutes of inactivity. The
// Tauri-side supervisor keeps a HashMap<server_id, Connection>.

use crate::mcp_types::{
    self, CallToolParams, InitializeResult, JsonRpcRequest, JsonRpcResponse, ListToolsResult,
...
struct Connection {
    ...
    next_id: u64,
    /// Cached `tools/list` from the server; refreshed on every Settings →
    /// "Test connection" but otherwise sticky for the supervisor's life.
    tools: Vec<McpTool>,
}
...
async fn spawn_server(spec: &McpServerSpec) -> Result<Connection, String> {
    ...
    let _: InitializeResult = serde_json::from_value(init).map_err(|e| e.to_string())?;
    notify(&mut conn, "notifications/initialized", Value::Null).await?;

    let tools_raw = request(&mut conn, "tools/list", Value::Null).await?;
    let tools: ListToolsResult = serde_json::from_value(tools_raw).map_err(|e| e.to_string())?;
...
pub async fn mcp_client_call(
    ...
    let params = CallToolParams { name: tool, arguments };
    let result = request(conn, "tools/call", serde_json::to_value(params).map_err(|e| e.to_string())?).await?;
    Ok(result)
}
