...
struct OpenAIMessage {
    ...
    /// Modeled as serde_json::Value so we emit either shape transparently —
    /// vision-capable backends accept the array form; text-only backends
    /// still consume the plain-string form unchanged.
    #[serde(skip_serializing_if = "Option::is_none")]
