...
export default function Home() {
  ...
  // Main-pane workspace switch (only when computer-use is enabled).
  const [workspace, setWorkspace] = useState<Workspace>('files');
  const showWorkspaceTabs = featureFlags.computerUseAgent;

  // Auto-switch to the Computer pane when a screenshot arrives, so the user
  // sees what the agent saw without manually clicking the tab.
  useEffect(() => {
    if (!showWorkspaceTabs) return;
    const handler = () => setWorkspace((w) => (w === 'files' ? 'computer' : w));
      ...
      <div className={styles.explorerContainer}>
        {showWorkspaceTabs && (
          ...
          >
            ...
            <Tab value="computer">Computer</Tab>
          </TabList>
        )}
        <div className={styles.workspaceBody}>
          ...
          {workspace === 'computer' && <ComputerView />}
        </div>
      </div>
